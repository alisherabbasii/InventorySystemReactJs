...
export const RecieptVoucher = () => {
  ...
  // const handleInputChangeTable = (index: any, field: any, value: any) => {
  //   setFormData((prev) => {
  //     const updatedEntries = [...prev.entries];
  //     updatedEntries[index] = { ...updatedEntries[index], [field]: value };
  ...
  const handleSave = async () => {
    // if (!formData.account || !formData.date || !partyId) return;
    ...
    try {
      const response = await createReceiptVoucher(obj);
      if (response.data) {
  ...
  return (
    <div>
      ...
      <div ref={contentRef} className="p-4">
        <h2 className="text-lg font-medium mb-4">Customer Payments</h2>
        ...
        <div className="grid grid-cols-2 gap-4 ">
          ...
          {/* <div>
            <label className="block text-sm mb-1">Customer No</label>
            <input
              type="text"
              className="border rounded px-2 py-1 w-full"
              value={customerNo}
